...
public class MainSoundScriipt : MonoBehaviour
{
    ...
    void OnActiveSceneChanged(Scene prevScene, Scene nextScene)
    {
        ...
        // TitleからResultSceneへ
        if (sceneName == "TitleScene" && nextScene.name == "ResultScene")
        {
            sceneName = "ResultScene"; // シーン名を更新
            bgm.Stop(); // BGMを停止
        }

        // ResultSceneからSceneSelectSceneへ
        if (sceneName == "ResultScene" && nextScene.name == "SceneSelectScene")
        {
            sceneName = "SceneSelectScene"; // シーン名を更新
            bgm.Play(); // BGMを再生
        }
